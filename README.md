...
the lab says so on its own front page. The other nine labs and the CV itself load
...
| `labs/` | Ten demo applications that run entirely inside a browser tab, with no server |
...
| `npm run test:labs` | The 5,657 assertions that ship inside the nine engine labs, run in real Chromium against the real pages — and the zero-network-call claim, policed at the network layer across every realm, page and worker alike |
