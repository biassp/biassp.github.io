...
the lab says so on its own front page. The other seven labs and the CV itself load
nothing.

| | |
|---|---|
| `index.html` | The CV itself, one file, including the live GitHub repositories feed |
| `labs/` | Eight demo applications that run entirely inside a browser tab, with no server |
...
| `npm run test:i18n` | Every `data-i18n` key on the two bilingual pages resolves in both languages |
| `npm run test:repos` | 14 jsdom tests over the GitHub repositories feed, including the fallback path when the API answers 403 |
| `npm run test:labs` | The 4,671 assertions that ship inside the seven engine labs, run in real Chromium against the real pages — and the zero-network-call claim, asserted rather than eyeballed |
