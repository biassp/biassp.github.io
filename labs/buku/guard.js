 ...
 *    blank a whole page.
 *
 * 2. The egress counter. This page holds a set of books — turnover, margins,
 *    owner's drawings, an NPWP. The page's CSP already sets connect-src 'none',
 *    so the browser refuses outbound connections whatever the JavaScript does;
 *    this wraps the five APIs that could try anyway and counts attempts, so the
...
(function () {
  ...
  var stored = readTheme('buku.theme') || readTheme('theme');
  ...
  function note(kind, target, asal) {
    ...
    if (window.BUKU_GUARD && typeof window.BUKU_GUARD.onchange === 'function') {
      try { window.BUKU_GUARD.onchange(); } catch (e) { }
    }
  }
