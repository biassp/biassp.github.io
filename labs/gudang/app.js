 ...
 *
 * Three things this file has to get right that are easy to miss:
 ...
 *     restored after it.
 *  2. A rebuild announces nothing to a screen reader. The posted document, the
 *     refusal and its reason, the recomputed HPP and the running total are all
 *     written into one polite live region.
 *  3. Where an action is denied, the control is still RENDERED and the reason
...
(function () {
  ...
  function invalidate() { state.hasil = {}; state.mundurHasil = null; }
  ...
  function hasil(metode) {
    ...
    if (!state.hasil[metode]) {
      state.hasil[metode] = L.replay(L.urut(state.db.buku.entries), {
        metode: metode, hargaAcuan: state.db.hargaAcuan
      });
    }
  ...
  function simpanEntry(e) {
    St.put('entri', { k: e.id, v: e });
    St.put('konfig', { k: 'buku', v: { seqBerikut: state.db.buku.seqBerikut, dokBerikut: state.db.buku.dokBerikut, tutup: state.db.buku.tutup } });
  }
  function simpanBuku() {
    St.put('konfig', { k: 'buku', v: { seqBerikut: state.db.buku.seqBerikut, dokBerikut: state.db.buku.dokBerikut, tutup: state.db.buku.tutup } });
  }
  ...
  function renderPesan(p) {
    ...
    box.appendChild(h('div', { style: 'margin-top:6px' },
      h('button', {
        class: 'btn small ghost', type: 'button', 'data-fkey': 'pesan:tutup',
  ...
  function renderBeranda(p) {
    ...
    rk.appendChild(box);
    rk.appendChild(h('p', { class: 'hint' },
      'Transfer masuk dan transfer keluar sengaja ditampilkan keduanya dan selalu sama besar (',
      D.rupiah(kons.transferMasuk), '): barang di perjalanan membawa biayanya, tidak dihargai ulang di gudang tujuan. ',
      'Identitas yang sama diuji ulang untuk kedua metode, sebelum dan sesudah sisipan bertanggal mundur, ',
      'di tab Uji.'));
    p.appendChild(rk);
  ...
  function renderKartu(p) {
    ...
    var rows = [], rusak = 0, mulai = Math.max(0, kartu.baris.length - 90);
    ...
    card.appendChild(h('div', { class: rusak ? 'tolak' : 'terima-ok' },
      h('b', { text: rusak ? rusak + ' BARIS MELANGGAR INVARIAN' : 'Invarian terpenuhi' }),
    ...
    [['fifo', hf, kf], ['rata', hr, kr]].forEach(function (t3) {
      ...
      g.appendChild(h('div', { class: 'metode-kartu' + (state.metode === m ? ' aktif' : '') },
        ...
        h('dl', { class: 'kv2' },
          ...
          h('dt', { text: 'Laba kotor' }), h('dd', { text: D.rupiah(hh.total.penjualanBruto - hh.total.hppJual) }),
  ...
  function renderHasilMundur(p, R) {
    ...
    var sum = h('div', { class: 'summary' },
      ...
      statTile('Berubah SEBELUM titik sisip', D.angka(diff.berubahSebelumBatas), diff.berubahSebelumBatas === 0 ? 'benar: hitung ulangnya lokal' : 'SALAH'),
    ...
    if (R.inc) {
      card.appendChild(h('div', { class: R.cocok ? 'terima-ok' : 'tolak' },
        h('b', { text: R.cocok ? 'Jalur inkremental cocok dengan replay penuh' : 'JALUR INKREMENTAL TIDAK COCOK' }),
        ' — ' + D.angka(R.dicek) + ' dokumen dibandingkan satu per satu. Hitung ulang dimulai dari snapshot ' +
        (R.inc.dariSnapshot ? D.periodeLabel(R.inc.dariSnapshot) : '(awal buku)') + ', ' +
        D.angka(R.inc.entriDipakaiUlang) + ' dokumen sebelum batas dipakai ulang apa adanya. ' +
  ...
  function renderPenerimaan(p) {
    ...
    if (pilih) {
      ...
      if (!state.terimaDraft || state.terimaDraft.poId !== pilih.po.id) {
        state.terimaDraft = { poId: pilih.po.id, qty: {} };
        ...
        (function (b) {
          ...
          rows.push(h('tr', null,
            ...
            h('td', null, numInput(state.terimaDraft.qty[b.sku], function (v) {
              state.terimaDraft.qty[b.sku] = Math.max(0, parseInt(v, 10) || 0);
              renderPanel('penerimaan');
            }, {
              'data-fkey': 'pn:qty:' + fkey(b.sku), min: 0, max: sisaB, step: 1,
      ...
      card.appendChild(h('div', { style: 'margin-top:8px' },
        h('button', {
          ...
          onclick: function () { postingPenerimaan(pilih); }
        }, 'Posting penerimaan')));
    } else {
  ...
  function postingPenerimaan(pilih) {
    var d = db();
  ...
  function renderKasir(p) {
    ...
    var cari = h('input', {
      class: 'cari', type: 'search', placeholder: 'Scan barcode / ketik SKU atau nama, lalu Enter…',
      value: state.sel.kasirCari, 'data-fkey': 'ks:cari', 'aria-label': 'Cari produk untuk keranjang'
    });
    cari.addEventListener('input', function () { state.sel.kasirCari = cari.value; renderPanel('kasir'); });
    cari.addEventListener('keydown', function (ev) {
      if (ev.key !== 'Enter') return;
      ev.preventDefault();
      var hit = cariProduk(state.sel.kasirCari);
      if (hit.length) tambahKeranjang(hit[0].sku);
    });
    ctl.appendChild(field('Cari produk', cari));
    card.appendChild(ctl);

    if (state.sel.kasirCari) {
      var hits = cariProduk(state.sel.kasirCari);
      /* In-stock first. A cashier scanning a shelf label wants the thing they
       * can actually sell at the top; an out-of-stock SKU still appears, marked
       * habis, because pretending it does not exist is worse than saying no. */
      ...
      for (var i = 0; i < Math.min(hits.length, 8); i++) {
        (function (pr) {
          ...
          chips.appendChild(h('button', {
            class: 'chip', type: 'button', 'data-fkey': 'ks:hit:' + fkey(pr.sku),
            title: q > 0 ? D.fmtPecah(pr, q) + ' tersedia di ' + gudangNama(gud) : 'Habis di ' + gudangNama(gud),
            onclick: function () { tambahKeranjang(pr.sku); }
          }, pr.sku + ' · ' + pr.nama + ' · ' + (q > 0 ? D.angka(q) + ' tersedia' : 'habis')));
        })(hits[i]);
      }
      ...
      (function (b, idx) {
        ...
        rows.push(h('tr', null,
          ...
          h('td', null, h('button', {
            ...
            onclick: function () { state.keranjang.splice(idx, 1); renderPanel('kasir'); }
          }, 'Hapus'))));
      })(state.keranjang[i], i);
    }
    ...
    ['tunai', 'transfer', 'qris'].forEach(function (j) {
      kanan.appendChild(h('div', { class: 'bayar-baris' },
        ...
        numInput(state.bayar[j], function (v) { state.bayar[j] = Math.max(0, parseInt(v, 10) || 0); renderPanel('kasir'); },
          {
      ...
      for (i = 0; i < nota.baris.length; i++) {
        (function (b) {
          ...
          rows3.push(h('tr', null,
            ...
            tdRp(hh ? hh.nilai : 0),
            h('td', null, numInput(state.returDraft.qty[b.sku] || 0, function (v) {
              state.returDraft.qty[b.sku] = Math.min(b.qty, Math.max(0, parseInt(v, 10) || 0));
              renderPanel('kasir');
            }, {
              'data-fkey': 'ks:returqty:' + fkey(b.sku), min: 0, max: b.qty, step: 1,
              'aria-label': 'Kuantitas diretur untuk ' + pr.nama
            }))));
        })(nota.baris[i]);
      }
      ret.appendChild(tableOf(['SKU', 'Nama', { label: 'Qty jual', num: 1 }, { label: 'Harga', num: 1 },
        { label: 'Nilai jual', num: 1 }, { label: 'HPP nota', num: 1 }, { label: 'Retur', num: 1 }], rows3, { minWidth: '820px' }));
  ...
  function tutupNota(bayarList, tot, bay) {
    ...
    for (i = 0; i < state.keranjang.length; i++) {
      ...
      var e = L.tambah(d.buku, {
        ...
        jenis: 'jual', dok: nota.id, nilaiJual: P.jumlahBaris(b2)
      });
      simpanEntry(e);
      nota.baris.push({ sku: b2.sku, satuan: b2.satuan, qtySatuan: b2.qtySatuan, qty: b2.qty, harga: b2.harga, diskon: 0, entryId: e.id });
    }
    d.kasir.push(nota);
  ...
  function postingRetur(nota) {
    ...
    for (var i = 0; i < nota.baris.length; i++) {
      ...
      if (q <= 0) continue;
      baris.push({ sku: b.sku, qtyBase: q, refEntry: b.entryId, nilaiJual: D.mul(q, b.harga) });
    }
  ...
  function renderLaporan(p) {
    ...
    for (i = 0; i < s.length; i++) {
      ...
      if (!mut[per]) mut[per] = { masuk: 0, keluar: 0, hpp: 0, jual: 0, beli: 0, adj: 0 };
      if (!hh) continue;
      if (e.arah > 0) mut[per].masuk += hh.nilai; else mut[per].keluar += hh.nilai;
      if (e.jenis === 'jual') { mut[per].hpp += hh.nilai; mut[per].jual += (e.nilaiJual || 0); }
    ...
    for (i = 0; i < perList.length; i++) {
      ...
      rows2.push(h('tr', null,
        ...
        h('td', { class: 'num', text: m.jual ? (D.divRound(D.mul(laba, 1000), m.jual) / 10).toFixed(1) + '%' : '—' })));
    }
    ...
    mutCard.appendChild(tableOf(['Periode', { label: 'Persediaan awal', num: 1 }, { label: 'Pembelian', num: 1 },
      { label: 'HPP', num: 1 }, { label: 'Penyesuaian', num: 1 }, { label: 'Persediaan akhir', num: 1 },
      { label: 'Penjualan', num: 1 }, { label: 'Laba kotor', num: 1 }, { label: 'Margin', num: 1 }],
      rows2, { minWidth: '960px' }));
    ...
    var jualPerSku = {};
    for (i = 0; i < s.length; i++) {
      if (s[i].jenis !== 'jual') continue;
      var hh2 = H.hasil[s[i].id];
      jualPerSku[s[i].sku] = (jualPerSku[s[i].sku] || 0) + (s[i].nilaiJual || 0);
    }
    var abc = [];
    ...
    var abcCard = h('div', { class: 'card' },
      ...
      h('p', { class: 'note' }, 'Diurutkan menurut nilai penjualan. Kelas A adalah SKU yang menyumbang 80% pertama, B sampai 95%, sisanya C — batas konvensional, bukan hasil optimasi.'));
