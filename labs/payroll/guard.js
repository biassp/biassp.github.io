 ...
 *    browser is set to block site data, and one unguarded read blanks a whole
 *    page. This site has shipped that bug once already.
 *
 * 2. The egress counter. This page holds salaries, PTKP status, NPWP and bank
 *    account numbers — a payroll file is the single most sensitive table in any
 *    company, and "trust me, it stays local" is not a claim anybody should
 *    accept without a number they can check. The page's own CSP sets
 *    connect-src 'none', so the browser refuses outbound connections whatever
 *    the JavaScript does; this wraps the five APIs that could try anyway and
 *    counts attempts, so the header shows a figure a visitor can compare
 *    against their own DevTools Network tab.
 *
 * Scope note, stated plainly: this is a claim about THIS page. The CV homepage
 * at biassp.github.io does call api.github.com for its repo feed. This lab
 * calls nothing, ever.
 */
(function () {
  'use strict';
  ...
  var stored = readTheme('payroll.theme') || readTheme('theme');
  ...
  function note(kind, target, asal) {
    ...
    if (window.PAYROLL_GUARD && typeof window.PAYROLL_GUARD.onchange === 'function') {
      try { window.PAYROLL_GUARD.onchange(); } catch (e) { }
    }
  }
